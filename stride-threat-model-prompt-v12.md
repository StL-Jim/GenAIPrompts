...
Maximum 20–25 threats in the final table. If more qualify, rank by risk severity (Likelihood × Impact) and select the top 20–25; record the count of lower-priority threats excluded in the Phase 2C Filtering Summary.
...
Column-by-column content comes from the merged threat table in `02b-threats.md` (which Phase 2C rolled into `02-threats.md`). Every column except `Disposition` and `DispositionRationale` is populated from the corresponding column in that table.
