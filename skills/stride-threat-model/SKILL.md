...
exists and is substantive; its pasted `readset.ps1 -Verify` output reads VERDICT: COMPLETE
(that reconciliation is tool-computed -- if the agent hand-wrote counts instead of pasting
the script output, treat the phase as unverified); and its depth verdict is not THIN. If any of those fail, re-dispatch it naming the shortfall -- nothing downstream
