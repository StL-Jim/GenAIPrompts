<!-- PROMPT VERSION: v24 (2026-07-15c) -- v22 base + PAIRED mechanical/comprehension discovery: step 7A mechanical candidate extraction (match values + quoted literals + key-value tokens -> tool-counted name list) AND step 7B mandatory targeted source reading over a tool-computed read set (top-10 match density + candidate-referenced files), with candidate triage informed by the reading. Also: path-preserving raw output, env-access pattern, no-truncation rule for accounting output, W4 asymmetric attestation, small v23 mechanical fixes. Deliberately EXCLUDES v23's disposition ledger and Phase 1 restructuring. If the version you are running does not match what the user expects, they may be on a stale copy. -->
PROMPT VERSION: v24 (2026-07-15c)
...
8. **Output directory layout:**
   ...
   {PROJECT_NAME}-threat-model/
     ...
     00-density.txt                    (Phase 0: per-file match counts -- basis of the tool-computed 7B read set)
...
7. **Identify primary language(s), framework(s), build system(s), and the concrete elements in scope** -- only from files you have directly observed. Look for `package.json`, `pom.xml`, `*.csproj`, `go.mod`, `requirements.txt`, `Cargo.toml`, `*.tf`, `Dockerfile`, `*.yaml` (k8s/helm), etc. Use `read_file` for each detection file and cite with evidence paths relative to the workspace root. "Identify" here means ENUMERATE BY CONCRETE IDENTITY, not "name the stack": list each service/process, each data store, each external integration, each secret location, and each pipeline/workflow you can see at scope level, by its actual name/id -- not a count. A generic quantifier standing in for a list ("several agents", "various services", "multiple buckets", "etc.") is a rule violation, not shorthand: if you are about to write "several X", stop and enumerate every X (use `Select-String` for the pattern to find them all, then read the relevant ranges). This is generic to any stack -- the element TYPES are fixed, the instances are whatever this repo actually contains.
   ...
   ```
   The no-whitespace and length bounds kill most prose junk mechanically (resource names never contain spaces; log messages almost always do). 7A is a FLOOR, not the method: it guarantees the names reach disk, but extraction judgment happens by READING (7B below). Do a quick first pass over the candidate list now, sorting each into obvious-noise / looks-like-a-resource / uncertain -- the FINAL triage table is written only after 7B, informed by what the files actually say. Never dismiss a candidate as noise from its name alone when a 7B read can settle it.

   7B. TARGETED SOURCE READING -- comprehension extraction over a TOOL-COMPUTED read set. The greps find WHERE and 7A finds NAMES; reading is where understanding happens, and field evidence is unambiguous that the strongest discovery runs won by reading the files where matches concentrate. This step makes that reading mandatory and bounds it. Build the read set mechanically -- both rules, no judgment about which files "matter":
   ```powershell
   $density = $all | Group-Object Path | Sort-Object Count -Descending
   $density | ForEach-Object { "$($_.Count)`t$($_.Name)" } |
     Set-Content ".\$PROJECT_NAME-threat-model\00-density.txt"
   $readSet = @($density | Select-Object -First 10 | ForEach-Object Name)
   ```
   - Rule (a), DENSITY: the top 10 files by match count (from $readSet above) are read, no exceptions.
   - Rule (b), CANDIDATE-DRIVEN: add every file (from 00-discovery-raw.txt) that references a candidate you sorted looks-like-a-resource or uncertain in 7A. This is what catches the long tail -- a single-reference table name in a low-density utility file pulls that file into the read set, where density alone would skip it.
   READ every file in the combined set -- fully, using Operating Rule 9 ranges for files over ~2000 lines -- and extract every resource BY CONCRETE IDENTITY: every store, bucket, table, queue, agent, external endpoint, integration, and secret surface the file defines or references, including ones no regex caught (dynamically-constructed names, resources described only in comments). Cross-check against the candidate list as you read: confirm real candidates, RESCUE any candidate the name made look like noise but the code shows is real, and add finds the extraction missed. THEN write the final triage table in 00-discovery.md, one row per candidate: `<candidate> | resource:<what it is> | dup:<canonical> | noise:<one word>` -- row count MUST equal the tool-computed candidate count (state both numbers). Also record the read-set reconciliation: `read set: <N> files (density 10 + <N> candidate-driven) | files read: <N> (MUST equal read-set size)`. Do NOT mark a file read that you did not read; if the session runs low, state which read-set files remain and finish them before writing 00-scope.md -- the read set is small by construction, typically 10-20 files.
   ...
   Write everything to `{PROJECT_NAME}-threat-model/00-discovery.md`: the exact patterns run with the per-pattern reconciliation above, the final candidate triage table (row count = tool-computed candidate count, state both), the 7B read-set reconciliation with per-file extraction notes (file, resources found, candidates confirmed/rescued), the list of documentation files read, and the merged DISTINCT list of external services / data stores / endpoints / integrations found (pattern finds + candidate resources + reading finds + doc finds), each with a `file:line` or `doc:section`. This file -- not memory or judgment -- is the authoritative "what exists" list that scope triages and Phase 1 inventories. Completeness is a property of WHICH searches were run, that every candidate has a triage row (counts stated), and WHICH docs were read -- all mechanical, all shown -- not of how thorough you feel.
...
Source reading: <N> of <N> read-set files read (density 10 + <N> candidate-driven) | resources found by reading: <N>
