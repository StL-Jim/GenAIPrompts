# Phase 4 fix -- build the diagram renderer and correct the data schema

Paste or copy this whole file to the machine holding the rebuilt STRIDE skill, and give it to
Claude Code with the skill directory in reach. It is self-contained: Part 1 is what to do,
Part 2 is the renderer specification Part 1 refers to. Nothing else is needed.

Wherever Part 2 says `BUILD_ROOT/stride-threat-model/`, read it as the directory your skill
actually lives in.


## Part 1 -- what to fix

Two things are wrong with Phase 4 of this skill. Fix both.

**Problem 1.** `references/phase-4.md` tells the agent to run `scripts/render-drawio.ps1` and
`scripts/validate-drawio.ps1`. Neither exists. Phase 4 therefore produces no diagrams at all.

**Problem 2.** The data-file schema in `references/phase-4.md` is wrong. It is missing six
fields the renderer reads, and it names the edge label field `label` when the renderer reads
`protocol`. Each of these degrades the diagram silently rather than failing.

### Step 1 -- build the two scripts

Build `scripts/render-drawio.ps1` and `scripts/validate-drawio.ps1` exactly to the
specification in Part 2 below.

Do its section 11 before you consider this step finished: render the sample, open the diagram,
and check it against the list there. Report what you SAW. "It rendered without error" does not
answer "does any edge cross a component".

### Step 2 -- correct the schema in references/phase-4.md

The data file is `{workspace}/{project}-threat-model/04-diagram-data.json`. The complete field
set:

    diagram: name, title, notes[]
    node:    id, label, kind, tier, tech, description, threat
    edge:    source, target, protocol, async, secure

    kind:   component | store | external | actor | process | dfdstore
    tier:   ACTORS | EDGE | APPLICATION | DATA | SECURED | EXTERNAL
    threat: P1 (red border) | P2 (orange border)

Four of these are easy to get wrong and each fails quietly:

- The edge label field is `protocol`, NOT `label`. Get it wrong and every flow renders
  unlabelled.
- `secure` is tested for FALSINESS, so an ABSENT `secure` renders the edge red and thick --
  the unencrypted-or-unauthenticated-flow signal. Every protected flow must carry
  `"secure": true` EXPLICITLY, or the whole diagram comes out red and says nothing.
- `threat` is what puts the P1/P2 borders on the threat diagram. Without it the diagram is
  just an architecture drawing.
- `tech` and `description` are the second and third label lines. Without them every box is a
  bare name and the C4 convention the diagrams are built on is lost.

Rewrite `phase-4.md` so the agent populates ALL of these, not just id/label/kind/tier. The
renderer computes every coordinate itself -- the data file must never contain `x`, `y`, `w`
or `h`.

### Step 3 -- check for dangling references

Grep the whole skill directory for `render-drawio`, `validate-drawio` and `04-diagram-data`.
Every hit must now point at something that exists. Check `SKILL.md` especially: its Phase 4
duty expects validation output in the returned banner.

### Report

What you changed, what the sample diagram looked like item by item against section 11 step 4,
and the Step 3 grep results.


---

## Part 2 -- the renderer specification
