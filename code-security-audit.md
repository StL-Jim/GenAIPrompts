...
- Note the threat model component count, trust boundary count, and threat counts (main table and Excluded Threats Ledger separately) for sanity checking later. Count ledger rows whose reason begins with `Code-level`, `Unverified`, or `Attested-mitigated` as seeded leads -- concerns the threat model routed to this audit to verify (an `Attested-mitigated (unverified)` row asks this audit to verify that a user-attested control actually exists and is effective in code/IaC).
