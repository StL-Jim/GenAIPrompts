...
PRIMARY OBJECTIVE
Perform a deterministic, multi-pass CODE SECURITY audit of the repository -- implementation defects first, architecture only where it creates security exposure -- using ONLY verifiable evidence from visible files and executed tools.
...
- COMPUTE (never eyeball) the counts recorded in coordination_mode.md, pasted:

```powershell
$T = "$PROJECT_NAME-threat-model"
$comp   = (Select-String -Path "$T\01-inventory.md" -Pattern '^\| C-\d').Count
$tb     = (Select-String -Path "$T\01-inventory.md" -Pattern '^\| TB-\d').Count
$main   = (Select-String -Path "$T\02-threats.md" -Pattern '^\| \d\d \|').Count
$ledger = (Select-String -Path "$T\02-threats.md" -Pattern '^\| EX-\d').Count
$seeded = (Select-String -Path "$T\02-threats.md" -Pattern '^\| EX-\d+ \|.*(Code-level|Unverified|Attested-mitigated)').Count
"components $comp, TBs $tb, main threats $main, ledger $ledger, seeded leads $seeded"
```

  If a table's format differs from these patterns, adapt the pattern and paste the adapted command and its output -- the requirement (per the NUMBERS ARE COMPUTED global rule) is that every count in coordination_mode.md is pasted command output, not a number produced by reading and counting mentally. Ledger rows whose reason begins with `Code-level`, `Unverified`, or `Attested-mitigated` are the seeded leads -- concerns the threat model routed to this audit to verify (an `Attested-mitigated (unverified)` row asks this audit to verify a user-attested control actually exists in code/IaC).
...
- Build:
  ...
  - high-risk zones
  - unknowns
- In COORDINATED mode, build the audit's inventory INDEPENDENTLY first, from this phase's own discovery (manifest + repo scan). Do NOT open the threat model's inventory until the audit's inventory is drafted -- reading it first anchors discovery to its contents, and the two pipelines are independent checking mechanisms only if discovery stays independent (an audit that inherits the threat model's inventory can never catch what the threat model missed, and discovery misses are the threat model's best-documented failure mode). THEN read `{PROJECT_NAME}-threat-model/01-inventory.md` and reconcile, recording a DISCOVERY DELTA section in 01_discovery.md: (a) elements the audit found that the threat model inventory lacks, (b) elements the threat model has that the audit's discovery did not find -- each a short named list with one line of evidence or explanation per element. ID policy: where an element matches, adopt the threat model's C-NNN/DS-NNN/EXT-NNN/TB-NNN verbatim (per C4 INPUT SCHEMA); audit-only elements get NEW IDs in the same scheme, flagged in the delta. The delta flows into the Phase 5 comparison's Coverage Analysis as discovery gaps.
...
- Account for every manifest file in every partition (see FILE COVERAGE ACCOUNTING below) -- ranking guides inspection depth in Phase 3A/4A, it must never cause a file to be silently dropped from consideration.

FILE COVERAGE ACCOUNTING (per partition, anchored to the manifest):

Tiering exists to focus depth of review, not to shrink the set of files Phase 3A/4A will look at. The accounting is PER PARTITION: `02_risk_prioritization.md` contains one tier table per partition, and the total each partition reconciles against is its manifest-derived file count from partition_plan.md (Phase 1 partition arithmetic) -- never a recalled number.

Tier table format (fixed, so the counts are tool-computable):
- One line per explicitly tiered file: `T<tier>: <manifest path> -- <one-line rationale>`. Tier 1 is the deepest review; tiers 1 and 2 are the files Phase 3A/4A will actually READ (this list is also the Phase 3A completion contract -- see the worker phases).
- One rolled-up bucket line per partition: `TB: <count> files -- <category description, e.g. test files, generated code, simple utility modules>`. The bucket count is COMPUTED as (partition manifest count minus explicitly tiered lines), never judged; bucket files get pattern-scan treatment only and need no per-file rationale. This keeps the accounting cheap regardless of partition size.

Before writing the completion banner, reconcile each partition with pasted commands: count its `T<tier>:` lines via Select-String over 02_risk_prioritization.md; take its manifest count from partition_plan.md. The banner carries one line per partition (see banner). If a tiered path is not in the manifest, or the arithmetic does not close, SAY SO and list the discrepancy -- the manifest makes the difference computable (Compare-Object over the two path lists). Do NOT loop re-deriving the table to force a match, and do NOT adjust any number without a command behind it: honesty about a gap is compliant; an uncomputed matching number is the violation.

Additionally compute each partition's tier-1+2 READING SET size (sum of the manifest byte column for its T1/T2 paths, pasted) and record it next to the tier table. This is the number compared against a Phase 3A session's reading capacity (~400KB guidance from Phase 1), and it confirms or updates the partition's expect-N-sessions annotation in partition_plan.md.
...
=== PHASE 2 COMPLETE: RISK PRIORITIZATION DONE ===
  audit_state/02_risk_prioritization.md
  partition <id>: <tiered> explicitly tiered + <bucket> bucket = <total> of <manifest count> manifest files; tier-1+2 reading set <bytes> bytes
  (one such line per partition, every number pasted from the reconciliation commands)
...
Same pattern as Phase 3A. In COORDINATED mode, apply the threat cross-reference procedure (from Phase 3A) to every architecture finding before writing it to disk. Architecture findings can match threat model threats too. Same `confirms` / `partial` / `unanticipated` semantics apply.

SEVERITY FLOOR, UNRELAXED: an ARCH finding must clear the Critical/High bar AS A SECURITY EXPOSURE. A quality observation (coupling, style, maintainability, operational convenience) with no Critical/High security consequence is not written up, regardless of its engineering merit -- this audit is not a design review, and diluting the registry with quality findings is exactly what the severity scope exists to prevent.

ANALYZE (architecture as it bears on security -- code-observable only):
- boundary violations (trust boundaries crossed, bypassed, or absent in code)
- shared state risks and race conditions with security consequence
- error handling and failure modes that fail open, leak information on error paths, or drop security checks under load
- isolation gaps between tenants, privilege levels, or security zones
- single points of failure IN SECURITY CONTROLS (an auth service, a policy enforcement point)
- edge cases in security-critical paths

In COORDINATED mode, architecture review is TARGETED, not open-ended: the threat model has already done top-down architecture reasoning over the full inventory, and re-deriving it here duplicates that work with less context. The primary 4A work is verification -- take the architecture-level threats and attack paths from `02-threats.md` that touch this partition and verify them against the code (the cross-reference procedure records the confirms/partial/contradicts outcome). Free-form architecture findings are limited to the security-relevant list above.

In STANDALONE mode ONLY, the broader design-review lens also applies (coupling/cohesion, dependency direction, resilience, operational fragility) -- no threat model exists behind a standalone run, so this phase is the only architecture examination the codebase gets. The severity floor above still applies unrelaxed.
...
- Section 1: Executive Summary
  ...
  - Every number in the counts table is computed and pasted BEFORE the table is written (NUMBERS ARE COMPUTED global rule). Threat-model-side totals come from coordination_mode.md (pasted at Phase 1). Audit-side tallies come from the registry, one count per threat_match value:

```powershell
foreach ($m in 'confirms','partial','confirms-seeded','contradicts-exclusion','excluded-by-design','unanticipated') {
  "$($m): " + (Select-String -Path audit_state\findings_registry.md -Pattern ('^threat_match: ' + $m + '$')).Count
}
"total findings: " + (Select-String -Path audit_state\findings_registry.md -Pattern '^id: F-').Count
```

    Percentages are derived from these pasted counts only. The attestation verified-vs-contradicted split: verified attestations come from the Phase 2 check results recorded in 02_risk_prioritization.md; contradicted ones are the contradicts-exclusion findings whose matched EX row's reason is Attested-mitigated -- count both from their on-disk sources.
...
- Section 6: Coverage Analysis
  - Every Section 6 number derives from already-pasted counts (coordination_mode.md totals and the Section 1 threat_match tallies) or from a new pasted command; component coverage is computed by cross-referencing IDs via Select-String (C-NNN occurrences in the threat model inventory vs in 02-threats.md and findings_registry.md), pasted. No Section 6 number is stated without a command behind it.
  ...
  - Discovery delta (reproduced from the DISCOVERY DELTA section of `01_discovery.md`): elements the audit's independent discovery found that the threat model inventory lacks are THREAT MODEL DISCOVERY GAPS -- list each with its evidence; elements the threat model had that the audit did not independently find are audit discovery gaps, listed likewise. These are pipeline-level findings about the toolchain itself, distinct from per-threat coverage, and they only exist because the audit's discovery runs independently before reconciling.
...
- PINNED TITLES (deliverable names are output contract, not style -- do not improvise them): 05_consolidated_report.html title and top heading = "Code Security Audit -- {PROJECT_NAME}"; executive_briefing.html = "Executive Briefing -- Code Security Audit -- {PROJECT_NAME}"; threat_audit_comparison.html (Phase 6) = "Threat Model vs Code Audit -- {PROJECT_NAME}". Improvised titles have drifted to architecture-first names that misrepresent the audit's mandate.
...
- Title heading = "Threat Model vs Code Audit -- {PROJECT_NAME}" (pinned, per HTML GENERATION REQUIREMENTS) and a brief introductory paragraph (1-2 sentences identifying this as the headline deliverable of the audit)
...
- cat: OWASP category (e.g., A01:2021, A03:2021), or `ARCH` for architecture findings from Phase 4A/4B that have no meaningful OWASP mapping but DO have Critical/High security consequence (e.g., missing isolation between privilege levels, a security-control single point of failure). Do not force-fit an OWASP category onto an ARCH finding -- and do not use ARCH as a home for quality observations that clear no security bar (see Phase 4A's severity-floor rule).
- sub: Subcategory (e.g., IDOR, SQL Injection, Missing Authentication; for ARCH findings e.g., Missing AuthZ Boundary, Shared-State Race, Security-Control SPOF)
