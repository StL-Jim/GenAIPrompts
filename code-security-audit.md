...
SEEDED_LEAD_COUNT: <N, ledger rows whose Exclusion Reason begins with Code-level or Unverified -- concerns the threat model deliberately routed to this audit as leads to verify, 0 if absent>
...
- Note the threat model component count, trust boundary count, and threat counts (main table and Excluded Threats Ledger separately) for sanity checking later. Count ledger rows whose reason begins with `Code-level` or `Unverified` as seeded leads -- concerns the threat model routed to this audit to verify.
...
1. Read the threats from `{PROJECT_NAME}-threat-model/02-threats.md`. The threat model contains TWO matchable structures, both in that file:
   ...
   - The EXCLUDED THREATS LEDGER (EX-NNN rows, from Phase 2C of the threat model; may be absent in models generated by older prompt versions). These are candidates the model considered but did not promote to the main table, each with a reason. Two reasons are leads routed to this audit to verify: `Code-level` (a specific implementation defect the model spotted) and `Unverified` (an architecturally plausible threat the model could not ground in its System Map -- these carry the confirming question to answer, and in older threat models lived in a separate Inferred table now folded into the ledger). Other reasons (`Fully mitigated`, severity/likelihood/scope) are exclusions, not leads.
...
- Section 1: Executive Summary
  ...
  - Counts table: total threats in the threat model main table, total ledger leads (Code-level + Unverified rows), total audit findings, threats confirmed, threats partial, seeded leads confirmed (includes former-Inferred Unverified rows the audit verified), exclusion contradictions, threats unconfirmed, audit unanticipated findings. Include percentages.
...
- Section 6: Coverage Analysis
  - Percentage of threat model entries with at least one confirming finding (severity-weighted and unweighted both shown). Report main-table coverage and ledger-lead coverage (Code-level + Unverified rows verified via confirms-seeded) separately.
