...
8. **Output directory.** All generated artifacts go under `.\{PROJECT_NAME}-threat-model\` inside the workspace (which is the source repo). Create it in Phase 0 and add it to `.git/info/exclude` so it is not accidentally committed to the source repo. The directory layout is:
   ...
   {PROJECT_NAME}-threat-model/
     ...
     02-threats.md                     (Phase 2D: consolidated, built from 02a/02b/02d)
     diagrams/
...
12. **STATE.md is the resume signal.** Every session -- including the very first -- begins by reading `{PROJECT_NAME}-threat-model/STATE.md` if it exists. This file is the authoritative answer to "where am I?" If it exists, jump to the next pending step rather than re-running completed work. If it does not exist (truly fresh run), start at Phase 0. Every phase and every Phase 2 sub-phase ends by updating STATE.md before printing its completion banner. The STATE.md schema is fixed:
    ...
    ## Resume Instruction
    <what the next session should do, e.g. "Begin at Phase 2D (Questions, Assumptions, Consolidation). Required rehydration: 01-inventory.md, 02a-context.md, 02b-threats.md.">
    ```
...
Maximum 20–25 threats in the final table. If more qualify, rank by risk severity (Likelihood × Impact) and select the top 20–25; record the count of lower-priority threats excluded in the Phase 2D Filtering Summary.
...
#### Threat Table Schema (16 columns)

This table is the canonical threat model. Every column has a clear job and pulls its weight. The table combines per-threat description (Title, Description, Evidence, etc.) with the attack-centric view (ThreatAgent, Asset, AttackSurface) in a single integrated row.

| Column | Description |
|--------|-------------|
| ThreatID | `0001`, `0002`, etc. Stable across re-runs. |
...
| Description | Why this threat matters for this component, how it would be exploited, and what the attacker gets. Combines what earlier versions called Why Applicable and Attack Path. Multi-sentence prose, but kept tight. |
...
| Disposition | Post-review tracking field. EMIT AS EMPTY STRING during generation. Reviewers fill this in after the threat model is reviewed (e.g., `Active`, `False Positive`, `Risk Accepted`, `Mitigated by Compensating Control`, `Duplicate of 0009`). |
...
(Count check: the schema lists 20 rows. Severity appears once as a column. ThreatID, Severity, Category, OWASP, Component, TrustBoundary, Title, ThreatAgent, Asset, Attack, AttackSurface, Impact, Description, Evidence, Likelihood, SecurityControl, ResidualRisk, Mitigation, Disposition, DispositionRationale = 20 columns total. The Disposition pair is the post-review block and stays empty during generation, so the agent is populating 18 columns of content during enumeration.)

Sort the table by Severity (Critical first), then by OWASP Top 10 item, then by ThreatID.
...
## Threat Table
| ThreatID | Severity | Category | OWASP | Component | TrustBoundary | Title | ThreatAgent | Asset | Attack | AttackSurface | Impact | Description | Evidence | Likelihood | SecurityControl | ResidualRisk | Mitigation | Disposition | DispositionRationale |
|----------|----------|----------|-------|-----------|---------------|-------|-------------|-------|--------|---------------|--------|-------------|----------|------------|-----------------|--------------|------------|-------------|----------------------|
| 0001 | Critical | Spoofing | A07:2021 | C-003 (Auth Service) | TB-002 | Session token replay due to absent token binding | External Attacker | AS-002 (Auth tokens) | Captured session cookie replayed against API (MITRE T1078) | External Interfaces | Confidentiality, Integrity | After intercepting a session cookie via XSS or network capture, attacker replays it against the API to impersonate the user. Edge terminates TLS, no token binding present, no anomaly detection. | [evidence: src/auth/session.go:120-158] | High | Partial -- TLS 1.3 on edge, no token binding | High | Implement RFC 8473 token binding; reduce session lifetime to 30 min; add anomalous-IP detection. | | |
```

Write the file with `create_new_file`. After writing, update STATE.md: mark `phase-2b: complete` with timestamp, set Last Completed Step, set Resume Instruction to `Begin at Phase 2D (Questions, Assumptions, Consolidation). Required rehydration: 01-inventory.md, 02a-context.md, 02b-threats.md.`
...
If `02-threats.md` does not exist or is empty, STOP and report the error -- Phase 2D did not complete consolidation and Phase 3 cannot proceed. Re-run Phase 2D (which will rebuild `02-threats.md` from the surviving 02a/02b/02d sub-files).
...
6. Questions and Assumptions -- content from the `02d-assumptions.md` portion of `02-threats.md`: Threat Filtering Summary, Excluded Threat Categories, Questions for Stakeholders, Assumptions Made.

#### Threats section format

The threats section uses a two-tier visibility pattern. Each threat is rendered as a primary row showing 8 visible columns. Below each row is a collapsible `<details>` element containing the remaining columns.

Visible columns (primary row): ThreatID, Severity, Component, Title, ThreatAgent, Asset, SecurityControl, Disposition.
...
The empty `--` option is selected by default so an unreviewed threat shows as visibly un-dispositioned.
...
Do NOT populate either control with values during generation. Both exist for the reviewer to fill in during stakeholder review.
...
`threats.csv` -- the canonical CSV export of the threat model. One row per threat. Header row required, columns in this exact order:

```
ThreatID,Severity,Category,OWASP,Component,TrustBoundary,Title,ThreatAgent,Asset,Attack,AttackSurface,Impact,Description,Evidence,Likelihood,SecurityControl,ResidualRisk,Mitigation,Disposition,DispositionRationale
```

Column names must match the header row above verbatim (spacing, capitalization, no spaces inside names) so any downstream Excel templates or scripts have a stable contract. Sort rows by Severity (Critical first, then High), then by ThreatID ascending.

Column-by-column content comes from the merged threat table in `02b-threats.md` (which Phase 2D rolled into `02-threats.md`). Every column except `Disposition` and `DispositionRationale` is populated from the corresponding column in that table.

`Disposition` and `DispositionRationale` are post-review tracking fields. EMIT THEM AS EMPTY STRINGS in every data row. Do NOT populate or guess at values during generation. They exist for the reviewer to fill in after the threat model is reviewed (e.g., `False Positive`, `Risk Accepted`, `Mitigated by Compensating Control`, `Duplicate of 0009`). Header row must include them; data rows must end in `,,` (two trailing empty fields).
...
Threat mapping: place threat IDs (`0001`, `0002`, ...) near affected components. Color-code component borders by highest threat severity present -- red for Critical, orange for High. The threat IDs ARE the cross-reference to the threat model table; no separate index needed.
