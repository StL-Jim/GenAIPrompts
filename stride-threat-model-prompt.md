<!-- PROMPT VERSION: v24 (2026-07-15d) -- v22 base; discovery restructured as TWO INDEPENDENT PASSES + REFINEMENT: Pass 1 source investigation (the primary method -- read entry points/main modules/docs deeply, extract by identity), Pass 2 mechanical sweep (nine patterns, one code block, three disk artifacts: raw w/ paths, density, candidates -- zero judgment), then a refinement that merges the pictures (unread top-density files read; every candidate accounted or read-then-decided; tool-computed counts). Also: no-truncation rule, W4 asymmetric attestation, small v23 mechanical fixes. Excludes v23's disposition ledger and Phase 1 restructuring. If the version you are running does not match what the user expects, they may be on a stale copy. -->
PROMPT VERSION: v24 (2026-07-15d)
